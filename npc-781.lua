...
function sampleNPC.onTickEndNPC(v)
	...
	--This whole thing is just a mess I know
	if data.state == STATE_SHOOT then
		...
		elseif data.shootTimer > 72 and data.shootTimer <= 74 then
            data.state = STATE_IDLE
			data.timer = 0
			data.shootTimer = 0
		end
		
		--Move it back and move it forward once hit
		if data.shootTimer >= 1 and data.shootTimer <= 23 then
			v.x = v.x + 2.1 * -v.direction
			data.walkTimer = 0
		elseif data.shootTimer >= 25 and data.shootTimer <= 72 then
			v.x = v.x + 1 * v.direction
			if v.direction == - 1 then
				v.animationFrame = math.floor(lunatime.tick() / 4) % 6 + 6
			else
				data.walkTimer = data.walkTimer + 1
				v.animationFrame = math.floor(lunatime.tick() / 4) % 6 + 18
			end
		end
	end
end
